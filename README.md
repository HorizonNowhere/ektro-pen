...
| **本地学习** | 没有"训练"按钮、没有"上传"键。你打字，本机的记忆就在长 |
| **链接分身**（可选） | 在 IME 设置登入 ektro → 把本地记忆带上来喂养你的 Twin（[ektroai.com](https://ektroai.com)） |
...
- **默认 100% 本地**。你打的字会被记录到本机 `%APPDATA%\Rime\ektro\` 的**明文 SQLite**，离线工作，**不联网传一个字**。你可以用任何 SQLite 客户端打开看、随时导出、随时删除。
- **登入 ektro 是可选升级路径**。如果你在 IME 设置里选"链接 ektro 账号"，从那一刻起，你的本地记忆会异步上传到 [ektroai.com](https://ektroai.com)，**唯一目的是喂养你本人的数字分身**。你可以随时在 IME 或 ektroai.com 解绑。不登入 —— EKTRO 完整工作，你的记忆始终在本机。
- **我们承诺**（写入 Ektro 宪法第五条禁令②）：你的数据不会被出售、不会用于训练任何公共/共享模型、不会被任何其他用户访问。
- **删除是定向的**：在 IME 里删某段 → 必须同时删云端；在 ektroai.com 里删某段 → 不会反向波及本地（本地是你的，云端是 Twin 训练副本，两边删除是两个独立的决定）。

详细规范见 [`docs/local-memory-schema.md`](docs/local-memory-schema.md) / [`docs/ektro-link-protocol.md`](docs/ektro-link-protocol.md) / [`docs/ime-ingest-contract.md`](docs/ime-ingest-contract.md) / [`docs/privacy-boundary.md`](docs/privacy-boundary.md)。
...
5. （可选）在 IME 设置里选"链接 ektro 账号"，把你的本地记忆带上来 [ektroai.com](https://ektroai.com) 喂养你的数字分身
...
淡灰下一句预测 · 本地明文 SQLite（可导出/删除）· **可选**：登入 ektro 后异步同步到 ektroai.com 喂养数字分身 ·
**可选**：分身演化为 AI Agent · 双进程崩溃隔离
