...
### D3. Commit capture 直接 link src-cpp 的 `ektro_sdk.h` C ABI

**选定**: 在 `SquirrelInputController.commitComposition:` 调用后, 调 src-cpp 的 C ABI `ektro_log_commit()` 写入 commit_log。Squirrel 是 ObjC++, 可直接 `#include "ektro/ektro_sdk.h"` 并静态 link `libektro.a`。

**已验证 (2026-05-22)**:
- src-cpp 在 macOS `cmake -DEKTRO_PLATFORM=macos` 编译通过, 43 GoogleTests 全过
- `ektro_sdk.h` C ABI 已含 `ektro_create / ektro_log_commit / ektro_set_password_field /
  ektro_memory_export / ektro_memory_clear / ektro_last_error` — commit capture 所需全部接口齐备
- C++ `memory_store.cpp` 已实现三层隐私拦截 (密码框 / 银行卡身份证 email 正则 / privacy_exclude)
- C++ schema 已升 v2 (device_link / sync_cursor / backfill_state), 与 Python 端对齐

**替代方案 (已否决)**:
- ObjC 复刻隐私拦截 — **错误**: ektro_log_commit 已含拦截, 复刻 = 重复造轮子 + 易与 C++ 版本漂移
- subprocess CLI 调 `python3 -m memory log-commit` — 每次 commit fork 进程, 延迟不可接受 (违反三戒之①)
- librime 层 hook — 太底层, weasel/squirrel 两端各自实现违反 DRY

**理由**: src-cpp C++ 层本就是为跨平台 SDK 设计 (CMakeLists 早有 EKTRO_PLATFORM 选项, ektro_sdk.h 是 C ABI)。macOS fork 的 commit capture 就是"调一个 C 函数", 零业务逻辑复刻。这是 src-cpp 存在的意义。
...
**选定**: 复用现有 Apple Developer 公司账号 (Organization), `Developer ID Application` +
`Developer ID Installer` 证书 `codesign + notarytool` 流程, `.pkg` installer 走自家网站分发.
注: 一个 membership 注册无限 App, 与该账号已上架的 App 互不冲突, 无额外 $99 开销.
...
4. ~~subprocess 还是嵌入式 sqlite3 直写 commit_log~~ — **已解决 (2026-05-22)**: 直接 link src-cpp `ektro_sdk.h` C ABI, 调 `ektro_log_commit`。C++ 已含隐私拦截 + schema v2, ObjC++ 零复刻。见 D3。
5. ~~Apple Developer 账号谁注册~~ — **已解决 (2026-05-22)**: 复用现有公司账号 (Organization,
   已付费有效)。待确认: 操作人在该账号的角色须为 Account Holder / Admin 才能创建
   Developer ID 证书。账号已上架过 App,membership 有效。
